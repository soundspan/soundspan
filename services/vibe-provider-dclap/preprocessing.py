...
import numpy as np
from settings import SAMPLE_RATE_HZ
...
def segment_audio(audio: object) -> list[object]:
    """Split audio into ten-second, 50%-overlap windows plus tail capture."""
    waveform = np.asarray(audio, dtype=np.float32)
...
def load_segmented_log_mels(
    ...
    """Load, quantize, segment, and convert one audio file into log mels."""
    librosa_backend = backend or _load_librosa()
    try:
        audio, _sample_rate = librosa_backend.load(audio_path, sr=SAMPLE_RATE_HZ, mono=True)
    except Exception as error:
        raise AudioDecodeError("Audio could not be decoded") from error
    quantized = int16_round_trip(audio)
    return [create_log_mel(segment, librosa_backend) for segment in segment_audio(quantized)]
