...
function HoverTooltip({ model }: { model: VibeMapViewModel }) {
    const track = model.gestures.hoveredId ?
        model.trackById.get(model.gestures.hoveredId) : undefined;
    ...
    return <div className="pointer-events-none absolute z-40 -translate-y-1/2"
        style={{ left: flip ? undefined : screen.x + 16,
            ...
            <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full flex-shrink-0"
                        style={{ backgroundColor: getMoodColor(track.dominantMood) }} />
...
function ContextHint({ model }: { model: VibeMapViewModel }) {
    const modeSheetOpen = model.shell.smallScreen && model.vibe.mode !== "explore";
    const hidden = model.shell.hintsDismissed || model.sweep.result || model.sweep.live ||
        model.escHintVisible || modeSheetOpen || model.data.loading || model.data.tracks.length === 0;
    if (hidden) return null;
    return <MapHintChip text={hintForMode(model.vibe.mode, {
        picking: model.vibe.journey?.picking, sweepArmed: model.sweep.brushArmed,
...
function FullscreenHint({ model }: { model: VibeMapViewModel }) {
    ...
    return <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 z-40">
