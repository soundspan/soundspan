...
function Decorations({ model }: { model: VibeMapViewModel }) {
    ...
    return (
        <MapDecorations
            viewport={model.camera.viewport!}
            posOf={model.layout.posOf}
            quantiles={model.data.quantiles}
            travel={
                travel
                    ? {
                          currentId: travel.currentId,
                          breadcrumbIds: travel.breadcrumbTitles.map(
                              (item) => item.id,
                          ),
                          onMapNeighbors: travel.onMapNeighbors,
                          onNavigate: travel.navigate,
                          onQueue: travel.queue,
                      }
                    : null
            }
            journey={
                journey
                    ? { fromId: journey.fromId, waypoints: journey.waypoints }
                    : null
            }
...
export function VibeMapCanvasStage({
    model,
    containerRef,
}: {
