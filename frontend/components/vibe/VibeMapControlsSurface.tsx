...
function MapViewControls({ model }: { model: VibeMapViewModel }) {
    ...
    return (
        <ViewControls
            ...
            onZoomOut={() => model.camera.zoomByCenter(1 / 1.3)}
            onReset={model.camera.resetView}
            layoutMode={model.layout.layoutMode}
            layoutDisabled={model.data.tracks.length === 0}
            onToggleLayout={model.layout.toggleLayoutMode}
            brushArmed={model.sweep.brushArmed}
            onToggleBrush={model.sweep.toggleBrush}
            canLocate={canLocate}
            locateHint={locateHint}
            onLocate={model.locateNowPlaying}
            canStartJourney={model.vibe.canStartJourney}
            journeyHint={journeyHint}
            onStartJourney={model.vibe.startJourney}
            queueOpen={model.aux.auxSurface === "queue"}
            onToggleQueue={() => model.aux.toggleAuxSurface("queue")}
            queueCount={Math.max(
                0,
                (model.audio.queue?.length ?? 0) -
                    (model.audio.currentIndex ?? -1) -
                    1,
            )}
            trailMode={model.trail.trailMode}
            ...
            trailEmpty={model.trail.trailIds.length === 0}
            onClearTrail={model.trail.clearTrail}
            onSaveTrail={model.trail.saveTrail}
            trailSaving={model.trail.trailSaving}
            ...
            onToggleFullscreen={() =>
                model.shell.setFullscreen(!model.shell.fullscreen)
            }
        />
    );
}
...
function QueueSurface({ model }: { model: VibeMapViewModel }) {
    ...
    return (
        <QueuePanel
            queue={model.audio.queue ?? []}
            currentIndex={model.audio.currentIndex ?? -1}
            onClose={model.aux.closeAux}
...
function SweepSurface({ model }: { model: VibeMapViewModel }) {
    ...
    return (
        <SweepChip
            ...
            capped={result.ids.length >= SWEEP_CAP}
            onPlay={model.sweep.play}
            onQueue={model.sweep.queue}
            onSave={model.sweep.save}
            saving={model.sweep.saving}
            onDismiss={model.sweep.dismissResult}
        />
    );
}
