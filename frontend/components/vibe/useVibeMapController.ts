...
function useMapCore(
    data: ReturnType<typeof useVibeMapData>,
    ...
    const controls = useMemo(
        () => ({
            playTrack: audioControls.playTrack,
            playTracks: audioControls.playTracks,
            addToQueue: audioControls.addToQueue,
        }),
        [
            audioControls.playTrack,
            audioControls.playTracks,
            audioControls.addToQueue,
        ],
    );
    const vibe = useVibeMode({
        trackById,
    ...
    const plan = useMemo(
        ...
        [audio.queue, audio.currentIndex, layout.posOf],
    );
    return {
        filters,
        layout,
...
function useMapInteraction(
    core: ReturnType<typeof useMapCore>,
    data: ReturnType<typeof useVibeMapData>,
    dims: ReturnType<typeof useMapDimensions>,
    shell: ReturnType<typeof useVibeMapShell>,
    ...
    const hitTest = useCallback(
        (clientX: number, clientY: number, radiusScale = 1) => {
            ...
            return findTrackAtScreenPoint({
                clientX,
                clientY,
                rect: container.getBoundingClientRect(),
                viewport,
                fitScale: fitViewport(dims).scale,
                tracks: data.tracks,
                positions: core.layout.positions,
                mask: core.filters.mask,
                radiusScale,
            });
        },
        [
    ...
    const locateTrack = useCallback(
        (id: string) => {
            if (locations.locateSearchResult(id))
                shell.setSpotlightHighlight(new Set([id]));
        },
...
export function useVibeMapController(props: VibeMapControllerProps) {
    ...
    useTravelCameraFollow({
        ...
        positionOf: core.layout.posOf,
        animate: core.camera.animateCameraTo,
        gestureActive: interaction.gestures.isGestureActive,
    });
    useJourneyCameraFrame({
        journey: core.vibe.journey,
        dims,
        viewportRef: core.camera.viewportRef,
        positionOf: core.layout.posOf,
        animate: core.camera.animateCameraTo,
    });
    const escHintVisible = useVibeMapEscape({
        sweepOpen: core.sweep.chipOpen,
        dismissSweep: core.sweep.dismissResult,
        auxSurface: core.aux.auxSurface,
        closeAux: core.aux.closeAux,
        mode: core.vibe.mode,
        exitMode: core.vibe.exitToExplore,
        fullscreen: shell.fullscreen,
        setFullscreen: shell.setFullscreen,
    });
    const presentation = deriveVibeMapPresentation({
        mode: core.vibe.mode,
        sweepHighlight: core.sweep.highlight,
        vibeHighlight: core.vibe.highlightIds,
        spotlightHighlight: shell.spotlightHighlight,
        trail: core.trail.trailPoints,
        plan: core.plan,
        filtersExpanded: shell.filtersExpanded,
        auxSurface: core.aux.auxSurface,
        sweepChipOpen: core.sweep.chipOpen,
    });
    const view = {
        props,
        shell: {
            ...
            setFullscreen: shell.setFullscreen,
            filtersExpanded: shell.filtersExpanded,
            setFiltersExpanded: shell.setFiltersExpanded,
            hintsDismissed: shell.hintsDismissed,
            dismissHints: shell.dismissHints,
            reducedMotion: shell.reducedMotion,
            smallScreen: shell.smallScreen,
        ...
        layout: core.layout,
        camera: {
            viewport: core.camera.viewport,
            zoomByCenter: core.camera.zoomByCenter,
