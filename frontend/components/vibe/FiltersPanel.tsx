    ...
    return (
        <div className={`${cardPos} pointer-events-none`}>
            ...
            >
                ...
                {/* Mood chips (subtractive toggles; shift-click solos). */}
                    ...
                    {moods.map((mood) => {
                        ...
                        return (
                            <button
                                ...
                                type="button"
                                onClick={(e) => {
                                    if (e.shiftKey) soloMood(mood);
                                    else toggleMood(mood);
                                }}
                                aria-pressed={active}
                                title={`${moodLabel(mood)} — click to toggle, shift-click to solo`}
                                className={`inline-flex items-center gap-1.5 h-9 px-3 rounded-full border text-xs transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/60 ${
                                    active
                                        ? "bg-white/10 border-white/15 text-gray-200 opacity-100"
                                        : "border-white/10 text-gray-300 opacity-35"
                                }`}
                            >
                                <span
                ...
                <DualRange
                    ...
                    highLabel="happy"
                    value={valenceRange}
                    onChange={setValenceRange}
                />

                <style>{`
                    .vibe-anim-in {
                        animation: vibe-anim-in 180ms ease-out both;
                    }
