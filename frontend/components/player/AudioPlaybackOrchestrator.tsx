...
import { playbackStateMachine, HeartbeatMonitor } from "@/lib/audio";
...
const TRACK_END_WATCHDOG_TIMEOUT_MS = 2_000;
const TRACK_END_WATCHDOG_BOUNDARY_SEC = 0.75;
...
interface OrchestratorEngineEventHandlers {
    ...
    handleManifestStall: AudioEngineEventHandler<"manifeststall">;
    cleanup: () => void;
}
...
export const AudioPlaybackOrchestrator = memo(
    function AudioPlaybackOrchestrator() {
        ...
        const segmentedStartupFallbackTimeoutRef =
        ...
        const transientTrackRecoveryLoadListenerRef = useRef<
            (() => void) | null
        >(null);
        ...
        const segmentedSessionCreateFallbackLastAttemptAtRef =
            useRef<number>(0);
        const segmentedChunkQuarantineRef = useRef<Map<string, number>>(
            new Map(),
        );
        ...
        const segmentedProactiveHandoffAttemptedTrackIdRef = useRef<
            string | null
        >(null);
        ...
        const segmentedProactiveHandoffCompletedTrackIdRef = useRef<
            string | null
        >(null);
        const segmentedProactiveHandoffLastSkipKeyRef = useRef<string | null>(
            null,
        );
        const startupSegmentedSessionRef =
            useRef<StartupSegmentedSessionSnapshot | null>(null);
        const startupSegmentedSessionInFlightRef = useRef<Set<string>>(
            new Set(),
        );
        ...
        const segmentedStartupRecoveryWindowStartedAtMsRef = useRef<
            number | null
        >(null);
        ...
        const segmentedHandoffListenerTimeoutRef =
            useRef<NodeJS.Timeout | null>(null);
        ...
        const emitSegmentedStartupTimeline = useCallback(
            (
                ...
                const timelineAnchorMs =
                    ...
                    current.loadAttemptStartedAtMs;
        ...
        const resolveHandoffLocalPositionSec = useCallback(
            (trackId: string, anchorPositionSec: number): number => {
                const livePositionSec =
                    resolveStartupSafeTrackPositionSec(trackId);
            ...
            ({
                ...
                const decision = resolveCorrelatedRecoveryResumeDecision({
                    ...
                    activeSessionTrackId:
                        activeSessionSnapshot?.trackId ?? null,
                ...
                if (!decision.matched) {
                    logPlaybackClientMetric("session.handoff_skipped", {
                        ...
                        activeSessionId:
                            activeSessionSnapshot?.sessionId ?? null,
                        activeTrackId,
                    });
                }
                return decision;
        ...
        const resolveBufferedAheadSec = useCallback((): number | null => {
            ...
            for (let index = 0; index < buffered.length; index += 1) {
                ...
                if (
                    currentTimeSec >= rangeStart &&
                    currentTimeSec <= rangeEnd
                ) {
            ...
            (
                ...
                segmentedHandoffListenerTimeoutRef.current = setTimeout(() => {
                    ...
                    logPlaybackClientMetric(
                        "session.handoff_listener_timeout",
                        {
        ...
        const validatePrewarmedSegmentedSession = useCallback(
            async ({
                ...
                if (
                    segmentedPrewarmValidatedSessionIdsRef.current.has(
                        sessionId,
                    )
                ) {
                    return "validated";
                ...
                if (
                    segmentedPrewarmValidationInFlightRef.current.has(sessionId)
                ) {
                    return "aborted";
                }
                ...
                if (
                    ...
                    priorSessionIdForKey !== sessionId
                ) {
                ...
                try {
                    ...
                    segmentedPrewarmValidatedSessionIdsRef.current.add(
                        sessionId,
                    );
                    ...
                    if (
                        ...
                        const abortReason = isTimeout
                            ? "timeout"
                            : typeof validationController.signal.reason ===
                                "string"
                    ...
                    segmentedPrewarmValidationInFlightRef.current.delete(
                        sessionId,
                    );
                    if (
                        segmentedPrewarmValidationAbortControllersRef.current.get(
                    ...
                    .then(async (session) => {
                        ...
                        if (validationResult === "failed") {
                            ...
                            if (
                                ...
                                logPlaybackClientMetric(
                                    "session.prewarm_skip",
                                    {
                                        trackId,
                                        sourceType:
                                            session.playbackProfile
                                                ?.sourceType ??
                                            session.engineHints?.sourceType ??
                                            context.sourceType,
        ...
        const ensureStartupSegmentedSession = useCallback(
            (
                ...
                startupSegmentedSessionInFlightRef.current.add(
                    startupSessionKey,
                );
                const startupPromise = api
                    .createSegmentedStreamingSession({
                        ...
                        startupCorrelationId:
                            startupMetadata?.startupCorrelationId,
                        manifestProfile: "steady_state_dual",
                    })
                    .then((session) => {
                        if (!isSegmentedSessionUsable(session)) {
                        ...
                        logPlaybackClientMetric("session.startup_ready", {
                            ...
                            sessionId: session.sessionId,
                            assetBuildInFlight:
                                session.engineHints?.assetBuildInFlight ===
                                true,
                            trigger,
                        });
                        return session;
            ...
            (reason: string): boolean => {
                ...
                void requestListenTogetherGroupResync(listenTogetherGroupId)
                    ...
                    .finally(() => {
                        ...
                        if (
                            currentRecoveryState.groupId ===
                            listenTogetherGroupId
                        ) {
                ...
                try {
                    ...
                    setStreamProfile({
                        ...
                        codec:
                            ...
                            "AAC",
                        bitrateKbps:
                            refreshedSession.playbackProfile?.bitrateKbps ??
                            null,
                    });
                    ...
                    const onRecoveryLoad = () => {
                        ...
                        logPlaybackClientMetric(
                            "session.handoff_circuit_recovered",
                            {
                                trackId,
                                sourceType:
                                    refreshedSession.playbackProfile
                                        ?.sourceType ??
                                    refreshedSession.engineHints?.sourceType ??
                                    segmentedTrackContext.sourceType,
                                previousSessionId,
                    ...
                    audioEngine.load(
                        ...
                        },
                        {
                            ...
                            format: "mp4",
                            startTimeSec:
                                correlatedStartTimeDecision.resumeAtSec,
        ...
        const attemptSegmentedHandoffRecovery = useCallback(
            async (
                ...
                const circuitAttempt =
                    segmentedHandoffCircuitRef.current.attempts;
                setIsBuffering(true);
                playbackStateMachine.forceTransition("LOADING");
                ...
                try {
                    ...
                    setStreamProfile({
                        ...
                        sourceType:
                            ...
                            segmentedTrackContext.sourceType,
                        codec:
                            handoff.playbackProfile?.codec?.toUpperCase() ??
                            "AAC",
                        bitrateKbps:
                            handoff.playbackProfile?.bitrateKbps ?? null,
                    });
                    ...
                    const onHandoffLoad = () => {
                        ...
                        logPlaybackClientMetric("session.handoff_recovered", {
                            ...
                            serverResumeAtSec: handoff.resumeAtSec,
                            resumeDeltaSec: Math.abs(
                                localPositionBeforeRecovery -
                                    handoff.resumeAtSec,
                            ),
                            latencyMs: Math.max(
                                0,
                                Date.now() - handoffStartedAtMs,
                            ),
                        });
                    };
        ...
        useEffect(() => {
            ...
            if (
                startupSegmentedSessionRef.current?.trackId !== currentTrack?.id
            ) {
            ...
            if (
                currentTrack?.id !== startupRecoveryAttemptedTrackIdRef.current
            ) {
        ...
        useEffect(() => {
            ...
            const heartbeatInterval = setInterval(() => {
                ...
                if (currentTrackRef.current?.id !== activeSession.trackId)
                    return;
                        ...
                        if (!guardedRefreshDecision.shouldTriggerRefresh) {
                            logPlaybackClientMetric(
                                "session.heartbeat_failure",
                                {
                                    ...
                                    failureThreshold:
                                        SEGMENTED_HEARTBEAT_FAILURE_THRESHOLD,
                                    guardedRefreshReason:
                                        guardedRefreshDecision.reason,
        ...
        useEffect(() => {
            const stableHandleTimeUpdate: AudioEngineEventHandler<
                "timeupdate"
            > = (payload) =>
                engineEventHandlersRef.current?.handleTimeUpdate(payload);
            const stableHandleLoad: AudioEngineEventHandler<"load"> = (
                payload,
        ...
        useEffect(() => {
            ...
            const handleVisibilityChange = () => {
                ...
                recoveryTimeoutId = setTimeout(() => {
                    ...
                    if (playbackStateMachine.getState() !== "RECOVERING")
                        return;
                    playbackStateMachine.forceTransition("LOADING");
                    audioEngine.play();
                }, 300);
        ...
        useEffect(() => {
            ...
            if (
                !nextTrack ||
                nextTrack.id === lastPreloadedTrackIdRef.current
            ) {
                return;
            }
                    ...
                    try {
                        ...
                        if (status.cached) {
                            ...
                            const onLoad = () => {
                                ...
                                if (
                                    seekOperationIdRef.current !== pollingSeekId
                                ) {
                                    podcastDebugLog(
            ...
            const handleSeek = async (time: number) => {
                ...
                if (playbackType === "podcast" && currentPodcast) {
                    ...
                    const executeSeek = async () => {
                        ...
                        try {
                            const status =
                                await api.getPodcastEpisodeCacheStatus(
                                    podcastId,
                                    episodeId,
                                );
                    ...
                    if (isLargeSkip) {
                        podcastDebugLog(
                            "seek: large skip, executing immediately",
                            {
                                timeDelta,
                                time,
                            },
        ...
        // Cleanup on unmount
        useEffect(() => {
            const prewarmedSegmentedSessions =
                prewarmedSegmentedSessionRef.current;
            const segmentedPrewarmInFlight =
                segmentedPrewarmInFlightRef.current;
