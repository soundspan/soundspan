...
const state = {
    homeShelves: [
        {
            title: "Featured",
            contents: [{ title: "item-1", playlistId: "PL123" }],
        },
    ] as {
        title: string;
        contents: {
...
test("FeaturedShelvesSection renders shelves section when shelves exist", async () => {
    const { FeaturedShelvesSection } =
        await import("../../features/explore/components/FeaturedShelvesSection");
...
test("FeaturedShelvesSection filters out video-only shelves", async () => {
    state.homeShelves = [
        {
            title: "Videos",
            contents: [{ title: "video-1", videoId: "VID456" }],
        },
    ];
    const { FeaturedShelvesSection } =
        await import("../../features/explore/components/FeaturedShelvesSection");
...
test("FeaturedShelvesSection filters out shelves with no navigable items", async () => {
    state.homeShelves = [
        { title: "Misc", contents: [{ title: "no-link-item" }] },
    ];
    const { FeaturedShelvesSection } =
        await import("../../features/explore/components/FeaturedShelvesSection");
    const html = renderToStaticMarkup(
    ...
    state.homeShelves = [
        {
            title: "Albums",
            contents: [
                { title: "album-1", browseId: "MPREb_abc", type: "album" },
            ],
        },
    ];
    const { FeaturedShelvesSection } =
    ...
    state.homeShelves = [
        {
            title: "Artists",
            contents: [
                { title: "artist-1", browseId: "UCabc", type: "artist" },
            ],
        },
    ];
    const { FeaturedShelvesSection } =
