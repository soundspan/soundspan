...
mock.module("@/lib/api", {
    namedExports: {
        api: {
            getTidalBrowsePlaylist: async () => {
                state.getPlaylistCalls += 1;
                return {
                    id: "playlist-1",
                    title: "Playlist",
                    trackCount: 0,
...
beforeEach(() => {
    state.routeId = "playlist-123";
    state.paramsCalls = 0;
    state.getPlaylistCalls = 0;
...
test("tidal playlist page renders deterministic loading state on server render", async () => {
...
test("tidal playlist page tolerates malformed encoded route id via decode fallback", async () => {
