...
import { resolveSegmentRepresentationIdFromUri } from "@/lib/audio-engine/segmentedRepresentationPolicy";
...
const readRuntimePositiveNumber = (key: string, fallback: number): number => {
...
const resolveUnsafeVhsBufferControls =
...
export class VideoJsSegmentedEngine implements AudioEngine {
    ...
    private usagePlayerHandler: ((event: VideoJsUsageEvent) => void) | null =
        null;
    private usageTechTarget: VideoJsEventTarget | null = null;
    private usageTechHandler: ((event: VideoJsUsageEvent) => void) | null =
        null;
    ...
    private qualityLevelAddHandler:
        | ((event: VideoJsQualityLevelEvent) => void)
        | null = null;
    private readonly representationQuarantineUntilMs = new Map<
        string,
        number
    >();
    ...
    private manifestStallDetectionTimer: ReturnType<typeof setTimeout> | null =
        null;
    ...
    constructor() {
        if (typeof window === "undefined" || typeof document === "undefined") {
            throw new Error(
                "VideoJsSegmentedEngine requires a browser environment.",
            );
        }
    ...
    private applyVhsBufferControls(
        ...
        const controlsAlreadyApplied = Object.entries(controls).every(
            ([key, value]) => {
    ...
    load(
        ...
        if (!resolvedSource.url) {
            throw new Error(
                "VideoJsSegmentedEngine.load requires a non-empty URL.",
            );
        }
        ...
        this.mediaElement.crossOrigin = withCredentials
            ? "use-credentials"
            : "anonymous";
        const explicitStartTimeSec = isFiniteNumber(
        ...
        this.seekTarget =
            explicitStartTimeSec ?? (shouldForceDashStartAtZero ? 0 : null);
        if (
            shouldForceDashStartAtZero &&
            isFiniteNumber(this.player.currentTime())
        ) {
    ...
    refreshManifest(): void {
        ...
        const vhs = tech?.vhs;
        const manifestLoaderCandidates: Array<
            VideoJsVhsManifestLoader | undefined
        > = [
        ...
        for (const loader of manifestLoaderCandidates) {
            ...
            if (typeof loader.load === "function") {
                ...
                sharedFrontendLogger.info(
                    "[SegmentedStreaming][ClientMetric]",
                    {
    ...
    quarantineRepresentation(
        ...
        this.scheduleRepresentationReenable(
            normalizedRepresentationId,
            safeCooldownMs,
        );
    ...
    private logVhsStartupConfig(): void {
        ...
        sharedFrontendLogger.info("[SegmentedStreaming][ClientMetric]", {
            ...
            startupGoalBufferSec:
                startupFragmentDurationSec *
                LOW_LATENCY_STARTUP_BUFFER_MULTIPLIER,
        });
    }

    private configureAudioOnlyMode(): void {
        const audioModePlayer = this
            .player as unknown as VideoJsAudioModeController;
    ...
    private getPlayerTechEventTarget(): VideoJsEventTarget | null {
        const tech = this.player.tech(
            true,
        ) as unknown as VideoJsEventTarget | null;
    ...
    private getPlayerQualityLevelList(): VideoJsQualityLevelList | null {
        const playerWithQualityLevels = this
            .player as unknown as VideoJsPlayerWithQualityLevels;
    ...
    private reconcileRepresentationQuarantine(nowMs: number = Date.now()): {
        ...
        for (let index = 0; index < levels.length; index += 1) {
            ...
            const quarantineUntilMs =
                this.representationQuarantineUntilMs.get(
                    levelRepresentationId,
                ) ?? 0;
        ...
        for (let index = 0; index < levels.length; index += 1) {
            ...
            const levelRepresentationId =
                this.resolveQualityLevelRepresentationId(level);
            if (!levelRepresentationId) {
                continue;
            }
            const shouldEnable =
    ...
    private pruneExpiredRepresentationQuarantine(nowMs: number): void {
        ...
        this.representationQuarantineUntilMs.forEach(
            (untilMs, representationId) => {
                if (untilMs > nowMs) {
                    return;
                }
    ...
    private maybeApplyVodFullTrackBufferControls(
        ...
        this.applyVhsBufferControls(
            ...
            },
            {
                ...
                fullTrackBufferPaddingSec:
                    DASH_VOD_FULL_TRACK_BUFFER_PADDING_SEC,
    ...
    private reconcileDashStartupLatencyTuning(
        ...
        if (liveState === "unknown") {
            ...
            if (trigger === "timeupdate") {
                ...
                if (
                    ...
                    currentTimeSec >=
                        DASH_UNKNOWN_LIVE_STATE_PROGRESS_SAMPLE_MIN_TIME_SEC
                ) {
        ...
        this.maybeApplyVodFullTrackBufferControls(
            sourceKey,
            "dash_vod_full_track",
        );
        if (this.vodFullTrackBufferAppliedForSourceKey !== sourceKey) {
    ...
    private getDashJsMediaPlayer(): VideoJsDashMediaPlayer | null {
        const tech = this.player.tech(
            true,
        ) as unknown as VideoJsDashRuntimeTech | null;
        if (!tech) {
            return null;
        }
    ...
    private bindPlayerEvents(): void {
        ...
        this.bindPlayerEvent("timeupdate", () => {
            ...
            if (
                ...
                Math.abs(timeSec - this.seekTarget) < 0.25
            ) {
                this.seekTarget = null;
            }
    ...
    private captureVhsResponseTelemetry(
        ...
        const kind = this.classifyVhsRequestKind(
            requestUri,
            request.requestType,
        );
        const statusCode = this.resolveStatusCode(request, response);
        const hasError =
            Boolean(error) || (isFiniteNumber(statusCode) && statusCode >= 400);
        const representationId =
            resolveSegmentRepresentationIdFromUri(requestUri);
        const sourceKey = this.getUsageEventSourceKey(source);
    ...
    private captureVhsQualityLevelTelemetry(
        ...
        sharedFrontendLogger.info("[SegmentedStreaming][ClientMetric]", {
            ...
            eventHeight: isFiniteNumber(eventQualityLevel?.height)
                ...
                : null,
            eventLevelId:
                typeof eventQualityLevel?.id === "string"
                    ? eventQualityLevel.id
                    : null,
            vhsBandwidth: vhs.bandwidth ?? null,
            vhsThroughput: vhs.throughput ?? null,
        });
    ...
    private getVhsRuntimeStatsSnapshot(): VideoJsVhsStatsSnapshot {
        ...
        return {
            bandwidth: isFiniteNumber(vhs.bandwidth)
                ? vhs.bandwidth
                : undefined,
            throughput: isFiniteNumber(vhs.throughput)
                ? vhs.throughput
                : undefined,
            ...
            mediaRequests: isFiniteNumber(vhs.stats?.mediaRequests)
                ...
                : undefined,
            mediaRequestsErrored: isFiniteNumber(
                vhs.stats?.mediaRequestsErrored,
            )
                ? vhs.stats?.mediaRequestsErrored
                : undefined,
            mediaRequestsTimedout: isFiniteNumber(
                vhs.stats?.mediaRequestsTimedout,
            )
                ? vhs.stats?.mediaRequestsTimedout
                : undefined,
            mediaRequestsAborted: isFiniteNumber(
                vhs.stats?.mediaRequestsAborted,
            )
                ? vhs.stats?.mediaRequestsAborted
                : undefined,
            mediaTransferDurationMs: isFiniteNumber(
                vhs.stats?.mediaTransferDuration,
            )
                ? vhs.stats?.mediaTransferDuration
                : undefined,
            mediaBytesTransferred: isFiniteNumber(
    ...
    private bindPlayerEvent(event: string, callback: () => void): void {
