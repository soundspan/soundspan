...
export interface CreateHowlerEngineOptions extends HowlerEngineAdapterOptions {
...
const toErrorPayload = (payload: unknown): AudioEngineErrorPayload => {
    ...
    const code =
        typeof recordPayload?.code === "string"
            ? recordPayload.code
            : undefined;
    const recoverable =
...
export class HowlerEngineAdapter implements AudioEngine {
    ...
    load(
        ...
        if (!resolvedSource.url) {
            throw new Error(
                "HowlerEngineAdapter.load requires a non-empty source URL.",
            );
        }
            ...
            case "volume":
                return {
                    ...
                    callbacks: [
                        (payload?: unknown) => {
                            const volume =
                                getNumericValue(getRecord(payload)?.volume) ??
                                0;
                            handler({ volume });
                        },
                    ],
