...
export class TauriNativeEngineAdapter implements AudioEngine {
    ...
    setVolume(value: number): void {
        ...
        if (!this.muted) {
            getTauriInvoke()
                .then((invoke) =>
                    invoke("native_audio_set_volume", { level: clamped }),
                )
                .catch(() => {});
        }
    }
    ...
    private async registerTauriListeners(): Promise<void> {
        ...
        await reg<NativeAudioTimeUpdate>(
            ...
            (payload) => {
                ...
                if (
                    this.seeking &&
                    this.seekTarget !== null &&
                    Math.abs(payload.position_secs - this.seekTarget) >
                        SEEK_STALE_WINDOW_MS / 1000
                ) {
                    return;
                }
        ...
        // buffering -----------------------------------------------------------
        await reg<NativeAudioBuffering>("native-audio-buffering", (payload) => {
