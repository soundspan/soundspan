...
export default function TidalPlaylistDetailPage() {
    return (
        ...
        >
            <TidalPlaylistDetailPageContent />
        </Suspense>
    );
}

function TidalPlaylistDetailPageContent() {
    ...
    useEffect(() => {
        ...
        async function fetchPlaylist() {
            ...
            try {
                const data = await api.getTidalBrowsePlaylist(playlistId);
                if (isActive) {
                    setPlaylist(data);
                }
            } catch (fetchError) {
    ...
    const isThisPlaylistPlaying =
        ...
        playlist?.tracks.some((t) => `tidal:${t.trackId}` === currentTrack?.id);
