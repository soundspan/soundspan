...
- On large libraries the Vibe Map's sample is now stable: the same songs stay on the map from one build to the next instead of a fresh random draw each time, and a song outside the sample is still placed when it plays, next to its closest sampled songs with a dashed beacon, so `Find on map` works for the whole library (#896).
...
- Lidarr queue removal now uses one explicit replacement policy, preserves duplicate-add recovery and operation-specific timeouts, and bounds cleanup retries so manual release selection suppresses auto-search while download retries remain responsive (#789).
...
- The administrator **Show user status from peers** toggle is removed. Peer presence now appears whenever federation is enabled and cached snapshots exist. Each user still controls their own visibility through **Share online presence**.
- Peer playlists now appear alongside local playlists in the sidebar list and on the Playlists page, badged with the server they come from. The sidebar filter menu gains a "Peer playlists" option and the Playlists page gains an All/Local/Peers source toggle (both only shown when federation is enabled); the Home "From your peers" shelf remains.
- The per-user `sharePresenceToPeers` and `sharePlaylistsToPeers` settings are removed. After upgrade, users who already share presence locally are also visible to federated peers: **Share online presence** now controls visibility everywhere, while **Share listening status** continues to control track details. Every public playlist is shared with peers that have `social:read`. Users who relied on the peer-specific opt-outs must turn off **Share online presence** or make affected playlists private.
...
- Explore Quick Start (except Shuffle All), Genre, and Decade tiles now open user-scoped generated playlists without autoplay. Reopening a tile returns its existing station playlist without rebuilding it. Station playlists stay out of web and Subsonic listings, while direct access remains available. Radio and ordinary edits share Playlist-first mutation ordering, and exhausted transient retries return a typed 503 response (#636).
...
- External-track album resolution now discovers a MusicBrainz release group through bounded MusicBrainz, Last.fm, and Deezer fallbacks, exposes distinct miss and timeout results through the authenticated metadata API, and links remote TIDAL and YouTube Music tracks to albums while preserving normalized-title convergence. Existing album identities remain unchanged; canonical release-group MBIDs are stored only on newly created album rows (#644).
...
- Federation cover-art lookups now reject placeholder and malformed release-group identities before Cover Art Archive or MusicBrainz access, preserve usable persisted covers before routing peer-owned misses through the federation proxy, clear stale native-cover state without redirect cycles or concurrent-cover loss, advertise proxyable peer art to Subsonic clients, and exclude peer albums from local cover enrichment (#680).
- Administrative user deletion now reserves the target with a durable deletion marker, fences new Listen Together memberships, and reconciles committed endings and departures through retry-safe publication and identity-based socket eviction on every backend replica before a guarded final delete. The cleanup replica evicts local socket and manager membership state directly, including when cluster sync is disabled, while each cluster (re)subscription audits its connected sockets against exact `(groupId, userId)` PostgreSQL membership pairs in 100-pair batches. Reconnect flaps coalesce into one active audit and one trailing rerun, and a timed-out non-cancelable Prisma batch retains that single-flight slot until it settles. Historical associations and a final all-user sweep revoke remote-only sockets even when local memory and Redis no longer show membership. One shared abortable deadline bounds collection, locks, reconciliation, and publication, abandons late idempotent effects, releases admission promptly, and prevents post-abort stages from overlapping a retry. Fenced socket mutations now treat a missing authoritative Redis snapshot as an ended group, reject malformed nested snapshots as retryable without poisoning local authority, and recheck the acting user's durable deletion marker inside the acquired lock before hydration. Marked users are revoked from stale connected sockets and local manager state before any mutation can republish them. Cleanup keyset pagination retains rows after earlier pages leave the mutating predicate. During a mixed-version backend window, admin user deletion retains pre-upgrade semantics until all pods run the new version (#667).
- Remote-only liked albums now load with user-scoped, canonical-deduplicated provider tracks, preserve tiddl-native TIDAL and YouTube artwork in playback, expose download/search controls only for real release-group MBIDs, and omit unsupported album-wide preference actions (#666, #665).
- Explore provider-backed sections now report initial-load failures in place without hiding retained data or triggering the global degraded-results banner, while library-backed failures—including radio genres and decades—retain the banner and Retry action. Provider preferences fail closed when settings are unavailable. The YouTube Music sidecar also shares one cancellation-safe library-playlists task per user and response shape, negative-caches upstream HTTP 400 failures per request shape for 10 minutes by default, invalidates old credential-era cache and flight state after credential restore, replacement, or removal, bounds provider transport calls, and drains its dedicated executor during shutdown. The backend preserves sanitized library-playlist 503/504 responses across the sidecar boundary.
- Listen Together group mutations now allocate and revalidate renewable fenced leases atomically, reject new HTTP and socket mutations through one command-owned shutdown admission gate, revalidate membership across socket attachment, preserve Redis and PostgreSQL snapshot version/time monotonicity, classify post-effect publication failures as non-retryable, reject stale or duplicate cluster events before observable emission through durable Redis authority, expire and evict local replay watermarks, coalesce cancellable bounded availability work without stale queue blockage, and bound ready-gate and publication retries with jittered deadlines. PostgreSQL membership transactions now validate the live lease immediately before advancing the durable group fence and again after their writes, so lease loss rolls back joins, departures, host transfers, and group endings. Membership snapshots and direct joined, left, presence, and revocation events carry an optional ordering version; new clients retain a per-group watermark, reject older membership state, accept version-less events until versioned authority is observed, and resync on later version-less membership events. During a mixed-version backend rollout, Listen Together coordination retains pre-upgrade unfenced semantics until all pods run the new version. Shutdown pauses now rehydrate and publish Redis authority under the group fence before PostgreSQL persistence, preserve DB-playing hydration normalization with a versioned pause, and abandon per-group lock waits inside the original drain deadline; cluster effects remain deferred through the post-compute authority check; direct membership fanout revalidates every irreversible stage; late ready-gate acquisition releases admission and local serialization before owner-token lease cleanup; and the monotonic drain deadline cannot renew after expiry. Startup now rejects rather than clamps unsafe lease timing and rejects invalid state-sync/store combinations (#662).
- YouTube Music playback works again under YouTube's SABR delivery enforcement: the sidecar now downloads complete tracks through yt-dlp's `web_safari` HLS path (with Deno-based JavaScript challenge solving) into a bounded least-recently-used local spool and serves player Range requests from the finished file, replacing progressive signed-URL proxying that YouTube no longer honors across range continuations. Concurrent requests for the same track share one download, and the spool is capped at 256 MiB by default (#651).
- Listen Together snapshot adoption now preserves members with live sockets on the receiving backend, removes only missing socketless members, reconciles one host from the persisted identity, restores that identity when a dropped host reconnects, serializes joins, reconnects, departures, and group termination, publishes only committed departure state across backend replicas, removes ended groups instead of recreating active snapshots, retries transient socket-join conflicts, keeps snapshot ordering watermarks monotonic across clock domains, and bounds cross-pod playback-time compensation to two seconds (#659).
- Listen Together membership mutations now capture live playback before committing, overlay only committed membership and host state, drain one shared per-group publication queue before unlocking, retry one transient post-commit publish failure, and emit membership-only or ended events when no authoritative snapshot exists. Exhausted reconnect rejoin attempts now request one bounded group resync while retaining error reporting (#661).
- Listen Together committed departures now remove omitted members exactly, revoke every remaining tab for the departed member, and transfer membership and host state across backend replicas without replacing playback. Revoked tabs receive a direct ordered notice before room removal and fully clear their client session, while publication retries skip completed membership stages instead of duplicating joined or left events. Snapshotless DB fallback joins emit presence-only updates, reconnects no longer duplicate joined events, and frontend resync acknowledgements time out within five seconds (#661).
- Listen Together hosts no longer skip ahead when their system clock runs ahead of the server: routine broadcast echoes do not position-correct the authoritative host, initial browser hydration adopts the compensated group position once, ready-gated starts resume a paused host without seeking, and reconnect reloads restore its captured local position. Follower latency compensation subtracts measured client-server clock offset, and the 5-second host heartbeat carries the group state version so the server discards stale position reports that would otherwise poison a just-started track. Version-less heartbeats from older clients keep their previous behavior.
...
- Provider outages no longer rapid-skip or repeatedly restart the queue because fatal local and Listen Together failures share one consecutive-error breaker. Error-driven same-track restarts re-arm progress confirmation so genuine recovery resets the breaker, while fresh host or polling media, repeat changes, and explicit manual playback actions also reset it. Tripped followers ignore same-track heartbeats until one of those reset actions occurs.
- Orphan album and artist cleanup now uses independent 100-row transactions with explicit wait and execution budgets, so large or contended runs retain successful batches and continue after an isolated batch failure.
- Stale, unliked, unplayable TIDAL and YouTube Music rows now expire through bounded daily garbage collection without pinning empty albums or artists. Album deletion uses one album-first lock order, retries catalog-link drift in a fresh transaction up to three times, atomically claims every eligible discovery row linked to the same catalog album, rolls back every claimed state when retention wins, preserves content when any linked row is liked or moved, protects rolling-deploy unlinked likes, canonicalizes legacy discovery ownership, carries ownership across release-group MBID corrections without recreating unliked ownership, and writes orphan federation tombstones through one shared path. Active mappings, playlists, likes, recent plays, liked albums, and manual parent overrides remain retained; each run drains up to 5,000 rows per provider and follows scheduled or manual zero-page purges (#649).
...
- Transient authentication-backend, rate-limit, network, and timeout failures no longer clear browser sessions when Explore triggers concurrent API requests; refresh retries now distinguish temporary unavailability from rejected credentials, apply bounded proactive backoff, discard outcomes and refuse to replay requests from earlier login sessions, and surface retryable partial-load failures in Explore. Dismissed partial-load notices stay dismissed when the same queries fail after a refetch and reappear when a different set of Explore queries fails (#635).
...
- Federation peer-health scrapes now bound PostgreSQL and Redis collection to five seconds, count and rate-limit warnings for collector failures, retain last-good samples with advancing sync lag during outages, expose never-synced consumers to freshness alerts without plotting epoch-scale dashboard lag, coalesce concurrent lease collection, keep timed-out collection single-flight until its underlying work settles, unref deadline timers, exclude revoked peers from metric series, document the 500-peer collection cap, and keep revoked, host-only, and consumer-only panel states direction-aware. Persisted federation diagnostics also redact common API credential forms and cannot mask an upstream stream error when either best-effort database update fails.
...
- OpenSubsonic classic play queues now exchange `current` as a song ID and retain legacy integer saves. Classic and index-based queue reads return an empty queue object with required ownership/change metadata when no state exists and retain that metadata when populated. The separate `indexBasedQueue` endpoints use the `playQueueByIndex.currentIndex` wire contract, reject invalid indices with error 10, and remap persisted current state after invisible entries are filtered (#626).
- OpenSubsonic song lists now expose authenticated-user star timestamps, exact 1–5 ratings, non-zero play counts, and derivable bitrates; album payloads also project star timestamps and play counts without per-item queries.
- Classic Subsonic `getArtistInfo` and `getAlbumInfo` endpoints now share the ID3 metadata implementation while returning their required classic envelope keys; classic artist info resolves artist, album, or song IDs, and classic album info resolves album or song IDs while the `*2` variants remain strictly typed.
- OpenSubsonic `estimateContentLength` requests retain exact lengths for fully materialized local transcodes and never substitute an unsafe estimate for original or Range responses.
...
- Library Health panel cache reads validate strict generation-bearing envelopes against the current Redis generation; unreadable or invalid generations, generation mismatches, and corrupt or stale-shaped payloads degrade to recomputation instead of serving stale data, blanking a panel, or hanging indefinitely.
- Library Health quality analytics now classify the scanner's installed-parser AIFF fallback labels, APE/Monkey's Audio, WavPack, and the lossy and lossless Matroska audio-codec families. New and rescanned tracks with missing codec metadata derive a stored label from the parser container or file extension before using the unknown-format fallback; existing rows keep their stored label because this release does not backfill them. Hybrid lossy WavPack remains an accepted report-only residual because the installed parser reports it as PCM. Analysis totals count every visible track with unknown statuses treated as pending, the storage summary reports the real artist count, and duplicate-cluster previews are capped at eight members while counts and sizes are computed from all loaded members and are exact within the loaded bound.
- Library Health cache invalidation now fails closed when Redis cannot advance its generation fence; a successful generation advance makes older envelopes unreadable and suppresses cache writes from older in-flight fills across backend replicas, while key deletion remains a best-effort optimization.
- Removed-track purge progress now mints a unique run ID when each root occurrence starts processing, persists it onto the job so retries reuse one owner, propagates it through continuations, and tolerates persisted legacy roots and old continuations. One-hour Redis leases, conditional refreshes that cannot recreate cleared owners, and arithmetic countdowns with periodic database corrections prevent reusable root job IDs and multi-page purges from corrupting live progress.
...
- Audio replacement and hash-unproven file changes now clear stale track loudness measurements. Album rollups recompute after removal, revival, rebind, reassignment, hard deletion, and same-hash duration corrections, with per-album locking keeping concurrent analyzer saves consistent; they exclude soft-removed tracks and clear when no active measurement remains. Same-hash revival retains its measurements because the audio is unchanged. Updated ReplayGain takes effect from the next playback of the track.
- Loudness backfill now retries transient analyzer failures through a jittered periodic sweep, atomically updates failure counters and their cooldowns, resets exhausted infrastructure-failure budgets after a cooldown, parks permanent content failures until the audio revision changes, expires superseded revision keys, and exposes bounded outcome counters.
...
- Federation requests now resolve and validate every peer address before each attempt, reject non-global IPv4 and IPv6 special-purpose ranges, and pin connections to validated addresses to prevent DNS-based SSRF and rebinding from receiving peer bearer tokens.
- OpenSubsonic trace logs now strip control characters and Unicode line separators, bound client-supplied fields, and keep form-posted credentials out of structured trace metadata.
...
- The Subsonic-compatible `/rest` surface now accepts form-encoded POST
  ...
  read-only allowlist; body parameters never override URL parameters and
  are never copied into loggable URLs. `getPodcasts`/`getNewestPodcasts`
