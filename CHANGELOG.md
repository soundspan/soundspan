...
- Audio replacement and hash-unproven file changes now clear stale track loudness measurements. Album rollups recompute after removal, revival, rebind, reassignment, and hard deletion; they exclude soft-removed tracks and clear when no active measurement remains. Same-hash revival retains its measurements because the audio is unchanged. Updated ReplayGain takes effect from the next playback of the track.
- Replacing or rescanning a track whose audio cannot be proven unchanged now clears its loudness measurements, and album loudness aggregates are recomputed on removal, hard deletion, revival, rebind, and album reassignment, with per-album locking keeping concurrent analyzer saves consistent. Identical-audio revivals keep their measurements, and a track already loaded in the player applies corrected gain from its next playback.
- Loudness backfill now retries transient analyzer failures through a jittered periodic sweep, resets exhausted infrastructure-failure budgets after a cooldown, parks permanent content failures until the audio revision changes, expires superseded revision keys, and exposes bounded outcome counters.
