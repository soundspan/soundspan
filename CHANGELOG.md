...
- The CI `Frontend Typecheck` job passes again: all 92 pre-existing `tsc --noEmit` errors in `frontend/tests/**` (the debt noted in the TypeScript-6 bump entry below and in the workflow's own comment, context #59) are resolved — extension-style imports in the four PR-#3 YouTube test files, incomplete `DiscoverPlaylist`/`DiscoverConfig`/`PlaylistPreview` fixtures, unnarrowed union access in `trackRef.test.ts`, boundary casts for the `serverBackendProxy` http fakes, a generic `read()` helper in `trackPrimitives`, and a local ambient augmentation (`tests/types/node-test-mock-module.d.ts`) declaring `mock.module()`'s new `exports` option, which the Node 24 runtime supports (nodejs/node#61727) but the installed `@types/node` 24.13.3 doesn't yet — to be dropped when `@types/node` catches up. Test behavior is unchanged (targeted suites: 67/67 unit, 69/69 component); this clears the way for flipping the typecheck jobs into the blocking ratchet.
