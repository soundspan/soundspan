...
- Provider outages no longer rapid-skip the queue because the consecutive playback-error breaker resets only after confirmed media progress.
- Provider outages no longer rapid-skip the queue because the consecutive playback-error breaker resets only after confirmed media movement, including during Listen Together recovery.
- Provider outages no longer rapid-skip the queue because fatal local and Listen Together failures share one consecutive-error breaker. Error-driven advances preserve the breaker across delayed group updates, while explicit manual selections and retries reset it.
- Provider outages no longer rapid-skip or repeatedly restart the queue because fatal local and Listen Together failures share one consecutive-error breaker. Error-driven follower resyncs preserve the breaker across delayed group updates, while fresh host or polling media, repeat changes, and explicit manual playback actions reset it. Tripped followers ignore same-track heartbeats until one of those reset actions occurs.
