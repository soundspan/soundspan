...
- External-track album resolution now discovers a MusicBrainz release group through bounded MusicBrainz, Last.fm, and Deezer fallbacks, exposes it through the authenticated metadata API, and links remote TIDAL and YouTube Music tracks to albums with canonical release-group MBIDs when available.
- External-track album resolution now discovers a MusicBrainz release group through bounded MusicBrainz, Last.fm, and Deezer fallbacks, exposes distinct miss and timeout results through the authenticated metadata API, and preserves normalized-title convergence for remote TIDAL and YouTube Music tracks. Existing album identities remain unchanged; canonical release-group MBIDs are stored only on newly created album rows.
...
- Transient authentication-backend, rate-limit, network, and timeout failures no longer clear browser sessions when Explore triggers concurrent API requests; refresh retries now distinguish temporary unavailability from rejected credentials, apply bounded proactive backoff, discard outcomes from earlier login sessions, and surface retryable partial-load failures in Explore. Dismissed partial-load notices reappear when a different set of Explore queries fails (#635).
...
- Federation peer-health scrapes now retain last-good samples during PostgreSQL or Redis outages, expose never-synced consumers to freshness alerts, coalesce concurrent lease collection, document the 500-peer collection cap, and keep host-only and consumer-only panel details direction-aware. Persisted federation diagnostics also redact common API credential forms and cannot mask an upstream stream error when their best-effort database update fails.
- Federation peer-health scrapes now bound PostgreSQL and Redis collection to five seconds, count and rate-limit warnings for collector failures, retain last-good samples with advancing sync lag during outages, expose never-synced consumers to freshness alerts without plotting epoch-scale dashboard lag, coalesce concurrent lease collection, exclude revoked peers from metric series, document the 500-peer collection cap, and keep revoked, host-only, and consumer-only panel states direction-aware. Persisted federation diagnostics also redact common API credential forms and cannot mask an upstream stream error when either best-effort database update fails.
- Federation peer-health scrapes now bound PostgreSQL and Redis collection to five seconds, count and rate-limit warnings for collector failures, retain last-good samples with advancing sync lag during outages, expose never-synced consumers to freshness alerts without plotting epoch-scale dashboard lag, keep timed-out collection single-flight until its underlying work settles, unref deadline timers, exclude revoked peers from metric series, document the 500-peer collection cap, and keep revoked, host-only, and consumer-only panel states direction-aware. Persisted federation diagnostics also redact common API credential forms and cannot mask an upstream stream error when either best-effort database update fails.
