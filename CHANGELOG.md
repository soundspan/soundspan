...
- Listen Together snapshot adoption now preserves members with live sockets on the receiving backend, removes only missing socketless members, reconciles one host from the persisted identity, restores that identity when a dropped host reconnects, serializes reconnects with atomic host transfers, falls back to active database membership when a socketless host is absent from memory, orders equal-version snapshots within the producer clock domain, and bounds cross-pod playback-time compensation to two seconds (#659).
