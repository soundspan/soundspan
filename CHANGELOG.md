...
- Audio replacement and hash-unproven file changes now clear stale track loudness measurements. Album rollups recompute after removal, revival, rebind, reassignment, and hard deletion, with per-album locking keeping concurrent analyzer saves consistent; they exclude soft-removed tracks and clear when no active measurement remains. Same-hash revival retains its measurements because the audio is unchanged. Updated ReplayGain takes effect from the next playback of the track.
