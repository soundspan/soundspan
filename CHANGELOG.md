...
- Listen Together hosts no longer skip ahead when their system clock runs ahead of the server: the host never position-corrects from its own broadcast echoes (it is the authoritative player), all latency compensation now subtracts the measured client-server clock offset (protecting followers with skewed clocks too), and the 5-second host heartbeat carries the group state version so the server discards stale position reports that would otherwise poison a just-started track. Version-less heartbeats from older clients keep their previous behavior.
