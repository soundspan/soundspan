...
- OpenSubsonic classic play queues now exchange `current` as a song ID and retain legacy integer saves. Classic and index-based queue reads omit their queue object when no state exists and include required ownership/change metadata when populated. The separate `indexBasedQueue` endpoints use the `playQueueByIndex.currentIndex` wire contract, reject invalid indices with error 10, and remap persisted current state after invisible entries are filtered (#626).
