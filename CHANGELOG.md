...
- The Vibe Map is now an interactive navigator instead of a static scatter plot. You can see where the currently playing track sits on the map (pulsing beacon + a locate button), watch your listening session draw a trail across it, and tame the 15k dots with per-mood show/hide chips (shift-click to solo), dual-thumb Energy and Mood (valence) range sliders, an animated "Spread" layout toggle that rank-equalizes the projection so dense clusters unfold without zooming, a fullscreen mode, zoom-scaled dot sizing, and a "spotlight" text search (`POST /api/vibe/search`) that glows matching tracks and dims the rest. Four new interaction modes: **Travel** — click any dot to walk the similarity graph outward with a mood compass (happier / sadder / calmer / more energetic) steering which neighbors are offered; **Journey** — pick a destination track or mood and get a numbered route of waypoint tracks drawn on the map, playable as a queue (new `POST /api/vibe/journey`, which walks CLAP embedding space from a start track toward a destination track or a mood centroid built from `MoodBucket` top tracks; new `GET /api/vibe/moods` lists which moods have enough analyzed tracks to serve as anchors); **Drift** — a one-click journey preset that queues a gradual 12-step slide toward a chosen mood; **Alchemy** — ctrl-click 2–10 tracks into a blending tray with per-ingredient weights and play the blend (wires the previously UI-less `POST /api/vibe/alchemy`). Also fixes two long-standing map annoyances: wheel zoom now anchors at the cursor, and a drag no longer fires click-to-play on release. A third design round makes the map the page: the Map tab now renders full-bleed (no more 60vh box) with a floating Explore/Map switcher, fullscreen layers above the player bar's seek strip (which used to bleed through), and on mobile every bottom-floating surface lifts clear of the mini player while the now-playing card gains an always-visible play/pause (in mobile fullscreen it is the only transport). Movement is now animated end to end — one camera owner coalesces wheel/drag/pinch input per frame (the wheel listener is native non-passive, so the page no longer scrolls or browser-zooms underneath), locate/reset/zoom-buttons fly on an eased log-scale path, travel recenters when its origin leaves the viewport's central band, journey routes frame themselves, and two-finger pinch zoom works on touch. Travel constellations no longer leave stale "strings": neighbour edges are origin-tagged (never drawn against a newer origin), decorations fade in, a click on clearly-empty canvas dissolves the constellation, the session trail draws only its 12 freshest hops and yields while a mode's own route is up, and the hover tooltip anchors beside the dot instead of bottom-center. Two new driver's-seat features: **Sweep** — hold Shift and drag (or arm the brush toggle; touch-friendly) to paint a stroke that collects up to 100 visible dots in first-touch order into a Play/Queue action chip; and a **flight plan** — a dashed polyline from the beacon through the next 10 on-map queue tracks, the forward-looking mirror of the session trail. A dismissable bottom-center hint chip teaches each mode's modifier verbs.
- New `GET /api/vibe/calibration` returns library-calibrated pairwise-distance quantiles — the p0-p100 percentiles of CLAP cosine distance over a random sample of up to 200 embedded tracks (Redis-cached 24h, keyed on the embedded-track count so it self-invalidates as the library grows) — so match percentages can be expressed as "closer than N% of random pairs in your library" instead of the fixed `1 - distance/2` mapping, which reads as inflated on libraries where unrelated tracks rarely exceed distance ~1.0 (e.g. two unrelated tracks showing a "50% match"). Returns `{ sampleSize: 0, quantiles: [] }` on libraries with fewer than 10 embedded tracks so the UI can fall back to the old linear mapping. `POST /api/vibe/journey` waypoints and `GET /api/vibe/path` steps also now carry the same `audioFeatures` (energy/valence/danceability/arousal) as `GET /api/vibe/similar/:trackId`, via their shared waypoint formatter.
- The vibe map's spotlight pill now doubles as a local track/artist finder: typed queries first search the already-loaded map in-memory (title/artist prefix and substring matches, ranked, up to 8 shown) and picking one flies the camera to that dot and glows it — no network round trip. The existing semantic `POST /api/vibe/search` "search this as a vibe" row stays the dropdown's last option, picked by Enter when there are zero local matches. Session-trail display now has three modes — On, Fade (age-fades and drops entries past 30 minutes), Off — plus a "Clear history" action, in a small popover off the trail button in the view-controls stack. Tracks whose CLAP mood confidence isn't high enough for a named mood (the backend's `neutral` fallback bucket) are now visible dots with a toggleable "Neutral" filter chip, instead of being permanently invisible and unfilterable on the map.
- Vibe map results can now be saved as playlists in one click (no naming prompt — rename later in the playlists UI): a "Save" button next to "Play journey" in the Journey panel (default name `Journey to <target> — <date>`, same track order as "Play journey"), a third sweep-chip action after a sweep-to-queue stroke (`Vibe sweep — <date>`), and "Save history as playlist" in the session-trail popover (`Vibe history — <date>`, the full stored trail oldest → newest, not just the drawn tail). Match percentages on the map surfaces (Travel neighbours, Journey waypoints, Alchemy blend results) are now scored against `GET /api/vibe/calibration`'s library-calibrated distance quantiles instead of the flat `1 - distance/2` mapping that floored unrelated pairs around 50% ("skindred shares 50% of a vibe with panic at the disco?!") — falls back to the old linear mapping (shown unlabeled) when the library hasn't got 10+ embedded tracks sampled. Travel neighbour rows gained a chevron "why this match" toggle: the calibrated sentence ("closer than N% of your library — same vibe") plus Energy / Mood / Groove / Intensity bars comparing the origin and candidate tracks' audio features (skipping any feature missing on either side). Travel-mode constellation edges now visibly encode true similarity too — stroke width/opacity scale with each neighbour's calibrated match percent, with today's fixed look kept as the ~75%-match anchor. A new "About this map" popover (Info button in the view-controls stack) explains the projection (a CLAP embedding, UMAP-projected — neighborhoods are meaningful, but long distances aren't proportionally "twice as different") and documents the mood-color legend, the beacon/trail/flight-plan line glyphs, and the click / shift-drag / ctrl-click / wheel-or-pinch / Esc gesture set.
...
- `POST /api/vibe/alchemy` no longer 500s when ingredient weights sum to zero
  or a non-finite value (e.g. `weights: [0, 0]`, which the existing
  floor-at-zero guard let through) — it now returns 400
  `weights must sum to a positive value` instead of feeding a NaN-filled
  blended embedding to pgvector. `POST /api/vibe/journey` in track mode now
  returns 404 `Destination track not found` instead of silently dropping the
  final waypoint when the destination track is deleted in the race window
  between its embedding lookup and its Track row fetch (`TrackEmbedding` has
  `onDelete: Cascade`, so the embedding can briefly outlive the track).
- Fixed three vibe-map Journey/Alchemy state-machine bugs where a half-built
  session could be silently lost or left showing stale data. Starting a new
  journey while already mid-journey (a different origin, or picking a new
  destination mood) no longer leaves the previous route's waypoints displayed
  against the new target — entering/re-entering journey now always clears the
  route and invalidates its fetch generation, not just the mode-change case
  the teardown effect covered. Ctrl/⌘-clicking a map dot while a journey panel
  was open used to silently discard the in-progress route by switching into
  Alchemy; it's now a no-op with a "Close the journey (Esc) to start
  blending" toast, and "Start a journey" is disabled with a "Close alchemy
