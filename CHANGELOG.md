...
- Untagged files in a folder whose tagged siblings created a real album no longer split into a duplicate same-titled album; they now title-match the artist's existing albums. Note: albums merged by the pre-1.5.1 scanner logic do **not** separate automatically on a routine rescan (unchanged files are skipped) — a forced full re-scan (or touching the files) is required for the corrected grouping to apply retroactively.
...
- The outbound-URL (SSRF) guard now **resolves DNS and range-checks the resolved IP** at every point the backend fetches a user-supplied URL (podcast/RSS feeds, the image proxy and its redirect hops, and admin connection tests). The previous check was string-only, so it was bypassed by alternate loopback encodings (`http://2130706433`, `0x7f000001`, `0177.0.0.1`, `127.1`) and by hostnames that resolve to a private/internal address; resolving via `getaddrinfo` normalizes those encodings and exposes the real target IP, which is then rejected if it falls in any private/loopback/link-local range. The fast string pre-check is unchanged for non-fetch validation sites.
