...
- Stale, unliked, unplayable TIDAL and YouTube Music rows now expire through bounded daily garbage collection without pinning empty albums or artists; active mappings, playlists, likes, and recent plays remain retained (#649).
- Album deletion and provider-track cleanup now recheck ownership before destructive writes, preserve files when database deletion fails, carry album ownership across release-group MBID corrections, and run provider garbage collection only after the terminal purge page.
- Album deletion and provider-track cleanup now recheck ownership and library promotion before destructive writes, preserve files and discovery links when guarded deletion loses a like race, carry existing ownership across release-group MBID corrections without recreating unliked ownership, and run provider garbage collection after scheduled and manual zero-page purges.
- Album deletion and provider-track cleanup now atomically claim discovery rows before destructive work, preserve files and links when a like wins the claim race, protect rolling-deploy unlinked likes, canonicalize discovery-liked ownership, discard legacy automated `enrichment` ownership unless backed by a like, carry ownership across release-group MBID corrections without recreating unliked ownership, and run provider garbage collection after scheduled and manual zero-page purges.
- Stale, unliked, unplayable TIDAL and YouTube Music rows now expire through bounded daily garbage collection without pinning empty albums or artists; active mappings, playlists, likes, recent plays, liked albums, and manual parent overrides remain retained. Each run drains up to 5,000 rows per provider, and orphan parent deletion writes federation tombstones through one shared path (#649).
- Stale, unliked, unplayable TIDAL and YouTube Music rows now expire through bounded daily garbage collection without pinning empty albums or artists. Album deletion atomically claims every eligible discovery row linked to the same catalog album, preserves content when any linked row is liked or moved, protects rolling-deploy unlinked likes, canonicalizes legacy discovery ownership, carries ownership across release-group MBID corrections without recreating unliked ownership, and writes orphan federation tombstones through one shared path. Active mappings, playlists, likes, recent plays, liked albums, and manual parent overrides remain retained; each run drains up to 5,000 rows per provider and follows scheduled or manual zero-page purges (#649).
