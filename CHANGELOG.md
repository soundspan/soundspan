...
- `.github/dependabot.yml` gained `ignore` rules encoding the 2026-07 Dependabot-wave review, so unmergeable majors stop re-proposing weekly: `@types/node` majors (backend npm — types track the node:24 runtime images, bumped with the base-image move, not ahead of it; #181), `typescript >= 7` (backend npm — ts-jest 29.4.11 peers `<7`, npm ci fails ERESOLVE; drop when ts-jest supports TS 7; #180), analyzer `redis >= 7` / `tensorflow >= 2.16` (pip — unresolvable on the Ubuntu 20.04/py3.8/essentia-tensorflow image, verified via `uv pip compile --python-version 3.8`; #165/#167), and clap `python` base minor/major bumps (docker — the hash-lock pins cp310 wheels, so a base bump without a lock regen breaks the build; the py3.14 move is a deliberate future PR; #164). The header's yt-dlp/ytmusicapi never-ignore rule is untouched.
