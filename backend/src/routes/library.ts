...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        // Map results - no on-demand image fetching for performance
        // Artists without images will show placeholders until enrichment completes
        const artistsWithImages = Array.from(artistsMap.values()).map(
            (artist) => {
                // Use override pattern: userHeroUrl ?? heroUrl
...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (includeTopTracks) {
            ...
            try {
                // Check cache first
                const cachedTopTracks =
                    await redisClient.get(topTracksCacheKey);
                ...
                if (unownedEntries.length > 0) {
                    ...
                    for (let i = 0; i < unownedEntries.length; i++) {
                        if (covers[i]) {
                            combinedTracks[
                                unownedEntries[i].index
                            ].album.coverArt = covers[i];
                        }
                    }
                }
...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (cursorLikedAtParam) {
            ...
            if (Number.isNaN(parsedCursor.getTime())) {
                return sendRouteError(
                    res,
                    400,
                    "Invalid cursorLikedAt timestamp",
                );
            }
        ...
        const hasTidal = hasConnectedProviderToken(
            userSettings?.tidalOAuthJson,
        );
        ...
        for (const mapping of mappings) {
            ...
            if (mapping.trackYtMusicId)
                tokens.push(`y:${mapping.trackYtMusicId}`);
            ...
            cursorLikedAt && cursorTrackIdParam
                ? deduped.filter(
                      (entry) =>
                          ...
                          (entry.likedAt.getTime() ===
                              cursorLikedAt.getTime() &&
    ...
    asyncHandler(async (req, res) => {
        ...
        if (url) {
            ...
            if (decodedId.startsWith("native:")) {
                ...
                const nativeCacheHit = resolveNativeCoverCacheHit(nativePath);
                if (nativeCacheHit) {
                    const canonicalNativePath =
                        nativeCacheHit.resolvedNativePath;
                    if (canonicalNativePath !== nativePath) {
                        const canonicalNativeCoverUrl = `native:${canonicalNativePath}`;
                        const canonicalAlbumId =
                            getAlbumIdFromNativeCoverPath(canonicalNativePath);
    ...
    async (req, res) => {
        try {
            ...
            if (playback) {
                ...
                const queryQuality = normalizeStreamingQuality(
                    req.query.quality,
                );
                let requestedQuality: StreamingQuality =
                    queryQuality ?? "medium";
            ...
            if (!fs.existsSync(metadataPath)) {
                return sendRouteError(
                    res,
                    404,
                    "Playback file not found on disk",
                );
            }
    ...
    async (req, res) => {
        try {
            ...
            if (artist.mbid && !artist.mbid.startsWith("temp-")) {
                try {
                    ...
                    if (lidarrResult.success) {
                        logger.debug(
                            `[DELETE] Lidarr: ${lidarrResult.message}`,
                        );
                        lidarrDeleted = true;
                    } else {
                        logger.warn(
            ...
            try {
                ...
                logger.warn(
                    "[DELETE] Could not delete OwnedAlbum records:",
                    err,
                );
            }
                    ...
                    similarTracks = similarTracks
                        .map((track) => {
                            const adjustedScore =
                                applyTrackPreferenceSimilarityBias(
                                    track.vibeScore ?? 0.5,
                                    similarTrackPreferenceScores.get(
                                        track.id,
                                    ) ?? 0,
                                );
                            return {
