    ...
    try {
        ...
        if (unknownAlbumTracks.length > 0) {
            ...
            unknownAlbumTracks.slice(0, 10).forEach((m) => {
                console.log(
                    `  - "${m.spotifyTrack.title}" by ${m.spotifyTrack.artist}`,
                );
            });
        ...
        preview.albumsToDownload.slice(0, 10).forEach((album) => {
            const mbidStatus = album.albumMbid
                ...
                : "NO MBID";
            console.log(
                `  - ${album.artistName} - "${album.albumName}" (${album.trackCount} tracks) [${mbidStatus}]`,
            );
        });
        if (preview.albumsToDownload.length > 10) {
            console.log(
                `  ... and ${preview.albumsToDownload.length - 10} more albums`,
            );
        }
