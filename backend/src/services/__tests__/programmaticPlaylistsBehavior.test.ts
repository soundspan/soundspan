...
const mockMoodBucketService = moodBucketService as jest.Mocked<
    typeof moodBucketService
>;
...
function makeTracks(
    ...
    const artistGroups =
        options?.artistGroups ?? Math.max(1, Math.ceil(count / 2));
...
describe("ProgrammaticPlaylistService behavior coverage", () => {
    ...
    it("generateAllMixes backfills from fallback generators, de-duplicates by type, and appends saved mood mix", async () => {
        ...
        expect(
            mixes.filter((mix) => mix.type === "fallback-duplicate"),
        ).toHaveLength(1);
        expect(mixes).toHaveLength(2);
        expect(mixes[1].id).toBe("saved-mood-id");
        expect(mockMoodBucketService.getUserMoodMix).toHaveBeenCalledWith(
            "user-1",
        );
    });
    ...
    it("generateAllMixes forceRandom mode is deterministic for a fixed seed input", async () => {
        ...
        for (const method of allMixGeneratorMethods) {
            jest.spyOn(service as any, method).mockResolvedValue(
                makeMix(method, `${method}-force-random`),
            );
        }
        ...
        expect(firstRun.map((mix) => mix.id)).toEqual(
            secondRun.map((mix) => mix.id),
        );
    ...
    it("generateTopTracksMix preserves ranked surviving tracks when catalog lookup misses played IDs", async () => {
        ...
        const fallbackTracks = makeTracks(30, "top-fallback", {
            artistGroups: 30,
        });
        let trackFindCalls = 0;

        (mockPrisma.play.groupBy as jest.Mock).mockResolvedValue(playStats);
        (mockPrisma.track.findMany as jest.Mock).mockImplementation(
            async () => {
                trackFindCalls += 1;
                return trackFindCalls === 1
                    ? rankedLookupTracks
                    : fallbackTracks;
            },
        );
        ...
        expect(mix!.trackIds.some((id) => id.startsWith("top-fallback-"))).toBe(
            true,
        );
    ...
    it("handles missing artist ids in diversify helpers via unknown fallback keys", async () => {
        ...
        const missingArtistTrack = makeTrack(
            "missing-artist-1",
            "seed-artist",
            {
                album: {
        ...
        const uniqueSecondPassNoAlbum = (
            service as any
        ).diversifyTracksUniqueFirst(
    ...
    it("forces diversify unknown-artist fallback branches with isolated artist-cap passthrough mocks", () => {
        jest.resetModules();
        jest.isolateModules(() => {
            const applyArtistCapMock = jest.fn(
                (tracks: any[], options?: any) => {
            ...
            const uniqueFirst = (
                isolatedService as any
            ).diversifyTracksUniqueFirst(unknownArtistTracks, 2, "iso-unique");
    ...
    it("generateGenreMix returns null when fallback genre scan fails to reach minimum", async () => {
        ...
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(4, "genre-fallback-low", { artistGroups: 4 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it("generateArtistSimilarMix returns null when similar artists in library have too few tracks", async () => {
        ...
        (mockPrisma.artist.findMany as jest.Mock).mockResolvedValue([
            {
                albums: [
                    { tracks: makeTracks(4, "similar", { artistGroups: 4 }) },
                ],
            },
        ]);
    ...
    it("generateFocusMix runs genre and audio fallback chain and fails when candidates remain under threshold", async () => {
        ...
        (mockPrisma.track.findMany as jest.Mock)
            ...
            .mockResolvedValueOnce(
                makeTracks(4, "focus-audio", { artistGroups: 4 }),
            );
    ...
    it("generateHighEnergyMix falls back to genre discovery and returns null when still under minimum", async () => {
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(3, "high-energy-audio", { artistGroups: 3 }),
            )
            .mockResolvedValueOnce(
                makeTracks(2, "high-energy-genre", { artistGroups: 2 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it("generateLateNightMix returns null when enhanced and standard pools are both too small", async () => {
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(3, "late-enhanced", { artistGroups: 3 }),
            )
            .mockResolvedValueOnce(
                makeTracks(7, "late-standard", { artistGroups: 7 }),
            );
    ...
    it("generateHappyMix exhausts standard and genre fallbacks before returning null", async () => {
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(2, "happy-enhanced", { artistGroups: 2 }),
            )
            .mockResolvedValueOnce(
                makeTracks(4, "happy-standard", { artistGroups: 4 }),
            )
            .mockResolvedValueOnce(
                makeTracks(3, "happy-genre", { artistGroups: 3 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it("generateMelancholyMix applies standard-mode filtering and still returns null when under threshold", async () => {
        ...
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(2, "melancholy-enhanced", { artistGroups: 2 }),
            )
            .mockResolvedValueOnce(standardCandidates)
            .mockResolvedValueOnce(
                makeTracks(2, "melancholy-genre", { artistGroups: 2 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it.each([
        ...
        async ({ label, run }) => {
            (mockPrisma.track.findMany as jest.Mock)
                .mockResolvedValueOnce(
                    makeTracks(3, `${label}-audio`, { artistGroups: 3 }),
                )
                .mockResolvedValueOnce(
                    makeTracks(2, `${label}-genre`, { artistGroups: 2 }),
                )
                .mockResolvedValueOnce([]);
    ...
    it("generateWorkoutMix executes all fallback layers and returns null when still under 15 tracks", async () => {
        ...
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(4, "workout-enhanced", { artistGroups: 4 }),
            )
            .mockResolvedValueOnce(
                makeTracks(3, "workout-audio", { artistGroups: 3 }),
            )
            .mockResolvedValueOnce(
                makeTracks(2, "workout-album-genre", { artistGroups: 2 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it("generateChillMix can fail after diversity filtering even when raw candidates meet minimum", async () => {
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(8, "chill-enhanced", { artistGroups: 1 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it("generateChillMix falls back to standard mode when enhanced coverage is insufficient", async () => {
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(5, "chill-enhanced", { artistGroups: 5 }),
            )
            .mockResolvedValueOnce(
                makeTracks(7, "chill-standard", { artistGroups: 7 }),
            );
        ...
        expect(mix).toBeNull();
        expect(
            (mockPrisma.track.findMany as jest.Mock).mock.calls[1][0].where,
        ).toEqual(
            expect.objectContaining({
                analysisStatus: "completed",
        ...
        expect(
            (mockPrisma.track.findMany as jest.Mock).mock.calls[1][0].take,
        ).toBe(100);
    });
    ...
    it("generateFocusMix uses album-pattern and audio fallbacks before returning a mix", async () => {
        ...
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(10, "focus-fallback-album", { artistGroups: 10 }),
            )
            .mockResolvedValueOnce([])
            .mockResolvedValueOnce(
                makeTracks(20, "focus-fallback-audio", { artistGroups: 20 }),
            );
    ...
    it("generateRoadTripMix returns null after exhausting tag, audio, and genre fallback paths", async () => {
        (mockPrisma.track.findMany as jest.Mock)
            .mockResolvedValueOnce(
                makeTracks(5, "road-tagged", { artistGroups: 5 }),
            )
            .mockResolvedValueOnce(
                makeTracks(4, "road-audio", { artistGroups: 4 }),
            )
            .mockResolvedValueOnce(
                makeTracks(3, "road-genre", { artistGroups: 3 }),
            )
            .mockResolvedValueOnce([]);
    ...
    it("generateCoffeeShopVibes returns null when strict criteria produce fewer than 8 tracks", async () => {
        ...
        const mix = await service.generateCoffeeShopVibes(
            "user-1",
            "2026-02-17",
        );
    ...
    it("generateCoffeeShopVibes enforces strict query and backfills to a 10-track mix", async () => {
        const initialTracks = makeTracks(8, "coffee-initial", {
            artistGroups: 4,
        });
        const backfillTracks = makeTracks(30, "coffee-backfill", {
            artistGroups: 30,
        });
        ...
        const strictQuery = (mockPrisma.track.findMany as jest.Mock).mock
            .calls[0][0];
        ...
        const backfillQuery = (mockPrisma.track.findMany as jest.Mock).mock
            .calls[1][0];
    ...
    it("generateMoodOnDemand maps ML mood requests into basic feature bounds when enhanced coverage is insufficient", async () => {
        ...
        const moodWhere = (mockPrisma.track.findMany as jest.Mock).mock
            .calls[0][0].where;
