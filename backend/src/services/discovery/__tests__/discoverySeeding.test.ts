...
describe("DiscoverySeeding", () => {
    ...
    describe("getSeedArtists", () => {
        ...
        it("should handle empty listening history by falling back to library", async () => {
            ...
            const artists = [
                {
                    id: "artist-1",
                    name: "Library Artist One",
                    mbid: "lib-mbid-1",
                },
                {
                    id: "artist-2",
                    name: "Library Artist Two",
                    mbid: "lib-mbid-2",
                },
            ];

            (mockPrisma.play.groupBy as jest.Mock).mockResolvedValue([]);
            (mockPrisma.album.groupBy as jest.Mock).mockResolvedValue(
                albumGroups,
            );
        ...
        it("should fall back to library when fewer than 5 recent plays", async () => {
            const recentPlays = [{ trackId: "track-1", _count: { id: 2 } }];

            const albumGroups = [{ artistId: "artist-1", _count: { id: 5 } }];
    ...
    describe("isAlbumOwned", () => {
        ...
        it("should return true when album found in previous discovery", async () => {
            ...
            (
                mockPrisma.discoveryAlbum.findFirst as jest.Mock
            ).mockResolvedValue({
                rgMbid: "album-mbid",
            });
        ...
        it("should return true when album available in Lidarr", async () => {
            ...
            expect(result).toBe(true);
            expect(mockLidarrService.isAlbumAvailable).toHaveBeenCalledWith(
                "album-mbid",
            );
        });
