...
describe("wrapNodeError", () => {
    const buildNodeError = (code: string, message = "node error") => ({
        code,
        message,
    });

    it("wraps ENOENT as recoverable FILE_NOT_FOUND", () => {
        const wrapped = wrapNodeError(
            buildNodeError("ENOENT", "no such file"),
            "readSong()",
        );
    ...
    it("wraps EACCES as fatal PERMISSION_DENIED", () => {
        const wrapped = wrapNodeError(
            buildNodeError("EACCES", "permission denied"),
            "openTrack()",
        );
    ...
    it("wraps EPERM as fatal PERMISSION_DENIED", () => {
        const wrapped = wrapNodeError(
            buildNodeError("EPERM", "operation not permitted"),
            "writeCache()",
        );
    ...
    it("wraps ENOSPC as transient DISK_FULL", () => {
        const wrapped = wrapNodeError(
            buildNodeError("ENOSPC", "no space"),
            "flushBuffer()",
        );
    ...
    it("wraps unknown codes as recoverable FILE_READ_ERROR", () => {
        const wrapped = wrapNodeError(
            buildNodeError("EIO", "input/output error"),
            "scanFiles()",
        );
