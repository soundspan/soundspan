...
1. Open `.agents-config/docs/ROUTE_MAP.md` (generated, canonical route map).
...
- Use `.agents-config/docs/RELEASE_NOTES_TEMPLATE.md` for release notes and GitHub release bodies; generate drafts with `npm run release:notes -- --version <X.Y.Z> --from <tag> [--to <ref>] [--output <path>]`.
...
- Keep `.agents-config/docs/FEATURE_INDEX.json` current as the machine-readable feature map (findability + ownership pointers).
...
- Keep `.agents-config/docs/TEST_MATRIX.md` current as the feature-to-targeted-test command map.
- Keep `.agents-config/docs/ROUTE_MAP.md` current as the generated backend/frontend route map (`npm run route-map:generate`).
- Keep `.agents-config/docs/JSDOC_COVERAGE.md` current for exported-symbol documentation drift checks (`npm run jsdoc-coverage:verify`).
- Keep `.agents-config/docs/LOGGING_STANDARDS.md` current for frontend/backend/python logging contract and compliance workflow.
...
- Whenever a feature is introduced, changed significantly, or removed, update or explicitly verify the impacted `.agents-config/docs/FEATURE_INDEX.json` entries in the same change set.
...
- All additional Git worktrees for this repo must be created under `../<repo-name>.worktrees`.
...
- Multi-agent queue usage:
  ...
  - Subagents should only receive atomic single-objective tasks and should not spawn additional orchestrators.
  - Default model routing should remain role/risk-based: orchestrator uses `gpt-5.3-codex` (`xhigh`), subagents use `gpt-5.3-codex` (`high`), and Spark is reserved for low-risk fast loops.
  - Spark low-risk loops should always include explicit verification commands in their task payload/result evidence.
  - `.agents/EXECUTION_QUEUE.json` is the single scope/queue authority for orchestrator and subagents.
  - `npm run agent:preflight` auto-syncs current/deferred `PLAN.json` tracks into queue items so every active/deferred feature has machine-readable queue representation.
  - `npm run agent:preflight` also backfills archived `PLAN.json` tracks into `.agents/EXECUTION_ARCHIVE_INDEX.json` and feature-sharded archive files.
...
- Managed workflow check/fix commands:
  - `npm run agent:managed -- --mode check`
  - `npm run agent:managed -- --fix --recheck`
- Template-impact declaration gate command:
  - `npm run agent:template-impact:check -- --base-ref origin/<base-branch>`
- Subagent payload-stub helper command:
  - `npm run agent:payload:stub -- --help`
...
- Simplified per-feature plan architecture (`.agents/plans/current/<feature-name>/`):
  - required: `PLAN.json` (machine-authoritative lifecycle contract)
  - optional legacy historical artifact: `PLAN.md` (non-authoritative context)
...
- Verbosity defaults for planning docs:
  - keep `Spec outline` sections concise (target `<= 220` words),
  - keep `Refined spec` sections concise but complete (target `<= 420` words),
  - keep each atomic implementation-task block concise (target `<= 160` words).
...
- Required end-of-feature procedure:
  ...
  7. Update `AGENTS.md`, `.agents-config/docs/AGENT_RULES.md`, `.agents-config/docs/AGENT_CONTEXT.md`, and `.agents-config/docs/CONTEXT_INDEX.json` if workflow expectations changed.
