...
- `rule_debugging_methodology_required`: Follow a structured debugging methodology (reproduce, isolate, hypothesize, verify root cause, validate fix) for bug remediation.
- `rule_verification_evidence_required`: Capture explicit verification evidence (commands run and key outcomes) for every completed change.
- `rule_code_review_standards_required`: Apply code review standards focused on correctness, regressions, maintainability, and test adequacy before closeout.
- `rule_security_review_required`: Perform a security review for changed surfaces, including auth boundaries, validation, and sensitive-data handling.
- `rule_pre_completion_checklist_required`: Complete a pre-completion checklist covering scope, verification, documentation, and residual risks.
...
- `rule_test_coverage_baseline_required`
