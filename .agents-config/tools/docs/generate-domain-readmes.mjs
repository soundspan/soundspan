...
const FEATURE_INDEX_FILE = ".agents-config/docs/FEATURE_INDEX.json";
  ...
  const rows = featureDirs.map((dirName) => {
    ...
    return [
      ...
      routes.length > 0
        ...
        : "(no routes mapped in `.agents-config/docs/FEATURE_INDEX.json`)",
    ];
  });
...
function renderFrontendFeatureDomainReadme({
  ...
  const routeLine =
    routes.length > 0
      ...
      : "No routes are currently mapped for this domain in `.agents-config/docs/FEATURE_INDEX.json`.";

  const commandLines =
    commands.length > 0
      ? commands.map((command) => `- \`${command}\``).join("\n")
      : "- Use `.agents-config/docs/TEST_MATRIX.md` to identify the right impacted test command(s).";
