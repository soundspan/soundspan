...
const SCRIPT_PROFILE_REQUIREMENTS = {
  "feature-index:verify": ["typescript", "javascript"],
  "jsdoc-coverage:generate": ["typescript", "javascript"],
  "jsdoc-coverage:verify": ["typescript", "javascript"],
  "openapi-coverage:generate": ["typescript-openapi"],
  "openapi-coverage:verify": ["typescript-openapi"],
  "logging:compliance:generate": ["typescript", "javascript"],
  "logging:compliance:verify": ["typescript", "javascript"],
  "route-map:generate": ["typescript", "javascript"],
  "route-map:verify": ["typescript", "javascript"],
  "domain-readmes:generate": ["typescript", "javascript"],
  "domain-readmes:verify": ["typescript", "javascript"],
};
...
function copyManagedFiles({ templateRoot, targetRoot, manifest, profiles, mode }) {
  ...
  for (const entry of entries) {
    ...
    const preserveExistingManagedFile =
      mode === "existing" &&
      (entry?.allow_override === true || authority !== "template") &&
...
function scriptIsActiveForProfiles(scriptName, activeProfiles) {
  const requiredProfiles = SCRIPT_PROFILE_REQUIREMENTS[scriptName];
...
function mergePackageScripts({ templateRoot, targetRoot, packageName, profiles }) {
  const activeProfiles = new Set(profiles);
  ...
  for (const [scriptName, scriptValue] of Object.entries(templateScripts)) {
    ...
    if (!scriptIsActiveForProfiles(scriptName, activeProfiles)) {
      const current = targetPackage.scripts[scriptName];
      if (typeof current === "string" && current === scriptValue) {
        delete targetPackage.scripts[scriptName];
...
function main() {
  ...
  if (packageMerge.removed.length > 0) {
    console.log(`- removed inactive profile scripts: ${packageMerge.removed.join(", ")}`);
  }
}
