...
node .agents-config/tools/index/indexer.mjs query "playback state route" --json

# Strict freshness gate (fail if git/index drift exists)
node .agents-config/tools/index/indexer.mjs query "openrouter config" --strict-fresh
```
...
- Config: `.agents-config/tools/index/index.config.json` (`isolation.mode: "branch-worktree"`)
...
Default scoring weights are configurable in `.agents-config/tools/index/index.config.json`.
