...
function resolveRepoWebUrl() {
    ...
    return normalizeRepoWebUrl(remoteUrl) ?? DEFAULT_REPO_WEB_URL_FALLBACK;
}
...
function buildReleaseSummary(counts, manualSummary) {
    ...
    if (counts.security > 0) {
        parts.push(
            formatCount(counts.security, "security update", "security updates"),
        );
    }
    if (counts.added > 0) {
...
function loadReleaseItemsFromChangelog(version) {
    ...
    if (!releaseSection) {
        fail(
            `Missing ${CHANGELOG_PATH} section [${version}]. Run node scripts/release/version-sync.mjs --write --version ${version} before generating release notes.`,
        );
    }
    ...
    for (const [heading, items] of bulletsByHeading.entries()) {
        if (
            items.length > 0 &&
            !supportedHeadings.has(normalizeHeading(heading))
        ) {
