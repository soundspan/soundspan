...
function parseReleaseImageTags(valuesYamlPath, releaseImageSections) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (
            ...
            !targetSections.has(currentTopLevelSection)
        ) {
            continue;
        }
...
function updateChangelogForRelease(releaseVersion) {
    ...
    try {
        ...
        fail(
            `${CHANGELOG_PATH} release-prepare update failed: ${error.message}`,
        );
    }
...
function verifyChangelogForRelease(releaseVersion) {
    try {
        ...
        fail(
            `${CHANGELOG_PATH} release-prepare validation failed: ${error.message}`,
        );
    }
}
...
function verifyHelmSurface(releaseVersion) {
    ...
    if (!fs.existsSync(chartYamlPath) || !fs.existsSync(valuesYamlPath)) {
        console.log(
            `Skipping Helm surface checks (missing chart files under ${CHART_DIR}).`,
        );
        return;
    }
...
function main() {
    ...
    if (mode === "write") {
        ...
        if (fs.existsSync(chartYamlPath) && fs.existsSync(valuesYamlPath)) {
            ...
            console.log(
                `Skipping chart update (missing chart files under ${CHART_DIR}).`,
            );
        }
