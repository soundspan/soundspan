...
const CHART_DIR = "charts/soundspan";
...
function assertPackageSurface(target, releaseVersion) {
  ...
  if (packageName !== target.expectedName) {
    fail(
      `${packageJsonPath} name mismatch. Expected "${target.expectedName}", found "${packageName}".`,
    );
  }
  ...
  if (!lockRoot || lockRoot.name !== target.expectedName) {
    fail(`${lockJsonPath} packages[""] name mismatch. Expected "${target.expectedName}".`);
  }
  ...
  return target.expectedName;
}

function verifyHelmSurface(releaseVersion) {
  const chartYamlPath = path.join(CHART_DIR, "Chart.yaml");
  const valuesYamlPath = path.join(CHART_DIR, "values.yaml");

  if (!fs.existsSync(chartYamlPath) || !fs.existsSync(valuesYamlPath)) {
    console.log(`Skipping Helm surface checks (missing chart files under ${CHART_DIR}).`);
    return;
  }
  ...
  const tags = parseReleaseImageTags(valuesYamlPath, RELEASE_IMAGE_SECTIONS);
  const missingSections = RELEASE_IMAGE_SECTIONS.filter((section) => !tags.has(section));
...
function main() {
  ...
  if (mode === "write") {
    ...
    if (fs.existsSync(chartYamlPath) && fs.existsSync(valuesYamlPath)) {
      ...
      console.log(`Skipping chart update (missing chart files under ${CHART_DIR}).`);
    }

    updateChangelogForRelease(releaseVersion);
  }

  const validatedPackageNames = PACKAGE_TARGETS.map((target) =>
    assertPackageSurface(target, releaseVersion),
  );
  verifyChangelogForRelease(releaseVersion);
  verifyHelmSurface(releaseVersion);
