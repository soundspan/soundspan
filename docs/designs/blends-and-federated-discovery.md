...
- **Privacy model**: the peer's `social:read` scope and each source user's **Share online presence** setting compose at export. The consuming server adds no administrator display gate: when federation is enabled, every successfully read cached snapshot appears in the Social roster. Presence sharing is per-user opt-in, while playlist federation is controlled by playlist publicness; both are documented in the federation privacy contract alongside the taste-vector rules.

> **Superseded — owner decision 2026-08-23:** The per-user peer-sharing gate was removed. Public playlists are exported to peers holding `social:read`; publicness is the only user-facing control.
