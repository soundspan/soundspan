...
Spec drafted 2026-08-16 for [issue #537](https://github.com/soundspan/soundspan/issues/537). Status: accepted and implemented through rollout phase 3. The registry, blue/green lifecycle, provider HTTP client, and backend-owned audio job path are live. DCLAP is the default provider in split Compose and AIO, remains an opt-in replacement in individual Helm mode, and the torch provider path was removed. Upstream DCLAP validation measured approximately 0.88 student-to-teacher fidelity. The Gate 2 run (see gate2-report-2026-08-17.md) confirmed the distinct `clap-music-audioset-dclap-student` space and selected the full re-embed migration path. Decision record: keep the LAION-CLAP `music_audioset` 512-dimension space active until cutover; adopt DCLAP's distilled ONNX student as the default provider through a full re-embed; formalize provider and embedding-space abstractions so future model changes are operational rollouts, not schema migrations.
...
- Vector queries always filter by their resolved space. The legacy global ANN index is removed. Each space with at least 1,000 vectors may have its own partial IVFFlat index; smaller spaces have no ANN index and use an exact scan. The lifecycle creates a missing valid index for the active space after it crosses the floor and drops a retired space's partial index after cleanup.
...
The 1,000-vector threshold is a conservative safety floor, not a claim that 224 lists are optimal for a corpus of that size. It supplies at least four training vectors per retained list, prevents empty or tiny-space training, and bounds exact scans to at most 999 vectors. Current pgvector guidance would choose fewer lists for a corpus this small; the established 224-list shape remains unchanged here to keep this correctness fix separate from production index retuning.
...
3. When coverage reaches `VIBE_SPACE_CUTOVER_THRESHOLD` (default 95%), the worker builds the target's partial ANN index only if it has at least 1,000 vectors, atomically marks the old space `retired` and the target `active`, then invalidates the active-space cache. A smaller target remains on exact scans until a later lifecycle tick observes that it crossed the floor and builds the index concurrently.
...
When the active space contains zero vectors and its durable `hadVectors` marker is false, the worker cuts over immediately without waiting for migration coverage. It does not train an IVFFlat index on an empty or undersized target. The embedding store sets the marker on the first vector, the migration backfills it for existing populated spaces, and the active-space lifecycle builds the target's partial index after backfill reaches 1,000 vectors. A later wipe therefore cannot masquerade as a fresh install. A genuinely fresh install may never deploy the teacher worker, so the guarded shortcut still closes the provider-only text-search dead window without permanently degrading ANN recall.

While the configured provider serves a registered migrating space, text search embeds and queries in that provider space, returning partial results that grow with the backfill; without a qualifying space-specific partial index, its exact scan has a fixed candidate bound and a transaction-local statement timeout. The 60-second provider-space cache continues routing to the same space across cutover and then refreshes its active status.

Vocabulary artifacts declare every `(family, checkpointHash)` identity whose text tower produced compatible vectors. Search blends the cached vocabulary only when one declared identity matches the resolved search space. Missing or mismatched identity metadata skips blending, increments a bounded-reason metric, and emits a rate-limited warning. The checked-in artifact was generated by the teacher text tower and declares both the teacher and DCLAP student identities because DCLAP packages that same text tower; the Gate 2 run recorded identical text-query rankings while keeping the distilled audio tower in a distinct space.
...
- ANN index tuning after initial rollout — the lifecycle retains the established `lists = 224` setting, delays training until a space has at least 1,000 vectors, and can be retuned from production recall measurements later.
