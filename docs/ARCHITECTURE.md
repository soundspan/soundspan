...
| ------------------------ | ----------------------- | -------------------------------------------------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| frontend                 | backend                 | HTTP (custom-server streaming proxy `/api/*`; Next route-handler fallback) | 3006                            | Bearer JWT (`Authorization` header, token in localStorage), plus `X-API-Key`; `?token=` query JWT only for media-element URLs | All API requests (admin-only surfaces such as Library Health included; Bull Board additionally accepts the path-scoped dashboard cookie minted from the Admin page)                 |
