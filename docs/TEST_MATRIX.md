...
- Run `npm run feature-index:verify` as a quick drift check for frontend feature coverage in `docs/FEATURE_INDEX.json`.
...
| `search-and-browse` | `npm --prefix backend test -- --runInBand src/routes/__tests__/searchRuntime.test.ts src/routes/__tests__/browseRuntime.test.ts`<br>`npm --prefix backend test -- --runInBand src/services/__tests__/searchService.test.ts` | `backend/src/routes/__tests__/searchRuntime.test.ts`<br>`backend/src/routes/__tests__/browseRuntime.test.ts`<br>`backend/src/services/__tests__/searchService.test.ts` |
