...
| ---------------- | -------------- |
| **2.3.3** | Usually none — plain rolling update; a small database change applies automatically. **Exception:** if you federate with a peer on a private LAN/VPN address, set `FEDERATION_ALLOW_PRIVATE_PEERS=true` first — outbound federation now blocks private addresses by default. If you are on 2.3.2, upgrade promptly: its audio-analyzer container could not start, so loudness measurement was paused until this release. |
