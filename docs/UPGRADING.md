...
| ---------------- | -------------- |
| **2.3.3** | Usually none — plain rolling update; a small database change applies automatically. **Exception:** if a federation peer is configured by a literal private LAN/VPN IP address (or `localhost`), set `FEDERATION_ALLOW_PRIVATE_PEERS=true` first — outbound federation now rejects literal private addresses by default (hostname-configured peers are unaffected). If you are on 2.3.2, upgrade promptly: its audio-analyzer container could not start, so loudness measurement was paused until this release. |
